...
export class LocalDataExampleComponent {
  usage = {
    html: `
    <amdg-grid
    *ngIf="showGrid"
...
`,
  ...
  initialHeadings: GridHeading[] = [
    ...
    {
      fieldName: 'personal_url',
      display: 'Personal Url',
      type: 'string',
      width: '200px',
      show: false,
    },
    {
      fieldName: 'top_level_domain',
    ...
    {
      fieldName: 'plant_family',
      display: 'Plant Family',
      type: 'string',
      width: '130px',
      show: false,
    },
  ...
  ngOnInit(): void {
    const demoGridUserPreference: GridHeading[] = JSON.parse(
      localStorage.getItem(this.GRID_ID) || '{}'
    );
    if (Object.entries(demoGridUserPreference).length > 0) {
      this.headings = demoGridUserPreference;
    } else {
